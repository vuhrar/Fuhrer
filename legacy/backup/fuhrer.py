# Archived original Streamlit UI (fuhrer.py)
# Saved here for reference. Do not run from project root; the active backend is app/
...
if st.session_state.show_panel == "full":
    with st.container():
        ...
        with t1:
            ...
            st.markdown(f"<div style='background:#2c2c2e;padding:12px;border-radius:8px;margin:6px 0;'><b>{selected.name}</b> {badge}<br><span style='color:#94a3b8;'>{selected.description}</span><", unsafe_allow_html=True)
...
if st.session_state.persona is None:
    ...
    with col1:
        st.markdown("<div class='persona-card'><div class='pc-icon'>⚖️</div><div class='pc-title'>المحامي والمستشار القانوني</div><div class='pc-sub'>متخصص في ا[...]")
        if st.button("بدء بصفتي محامي", key="btn_lawyer", use_container_width=True): select_persona("lawyer")
    with col2:
        st.markdown("<div class='persona-card'><div class='pc-icon'>💼</div><div class='pc-title'>المستشار العمالي</div><div class='pc-sub'>خبير في الامتثال، الت[...]")
    ...
    st.markdown(f"<div style='text-align:center;margin-bottom:20px;'><span style='background:#2c2c2e;border:1px solid #c9a84c;color:#c9a84c;padding:5px 20px;border-radius:20px;font-weight:700;'>{name}</span></div>", unsafe_allow_html=True)
