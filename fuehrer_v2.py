...
with t_ai:
    ...
    user_inp = st.text_area(
        "سؤالك القانوني",
        value=st.session_state.pop("pending_q","") if "pending_q" in st.session_state else "",
        height=90,
...
""" + "".join(f"• [{m['category']}] {m['text'][:80]}\n" for m in st.session_state.memory[-5:])
    ...
    with rp_c2:
        ...
        reqs      = st.text_area("الطلبات", "إلغاء القرار والتعويض", height=60)
...
with t_mem:
    ...
    with st.expander("➕ إضافة ذاكرة جديدة"):
        mt  = st.text_area("النص", height=70, placeholder="مثال: الموكل يعمل منذ 2019 براتب 8000 ريال")
...
with t_law:
    ...
    with lc2:
        ...
        ma_text = st.text_area("نص المادة", height=80, key="ma_t")
...
""")
        ...
        if st.button("⬇️ تحميل النموذج"):
            try:
                ...
                st.error(f"❌ {e}")
        hf_text = st.text_area("النص", height=80)
