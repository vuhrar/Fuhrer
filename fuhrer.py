# fuhrer.py — نقطة الدخول الرئيسية
# السطران الأولان يضبطان مسار Python عشان يلقى الوحدات بنفس المجلد

import sys
import os

# يضيف مجلد هذا الملف نفسه لمسار البحث — يحل ModuleNotFoundError
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

# الآن يستورد كل شي بأمان
...
with st.sidebar:
    ...
    with c1:
        if st.button("لوحة\nالتحكم", key="btn_dash", use_container_width=True):
    ...
    with c2:
        if st.button("الإعدادات", key="btn_settings", use_container_width=True):
            ...
            st.rerun()
    with c3:
        if st.button("الاتصال\nبالخادم", key="btn_conn", use_container_width=True):
    ...
    if st.button("🧭 تحليل شامل — 18 محوراً", key="deep_btn", use_container_width=True):
...
<div class='app-header'>
  <h1>Führer</h1>
  <span class='badge' style='border-color:{persona_color};color:{persona_color};'>
    {persona_label}
  </span>
</div>
""", unsafe_allow_html=True)
...
if panel == "dashboard":
    ...
    <div class='stat-grid'>
      <div class='stat-box'>
        ...
        <div class='l'>الجلسات المحفوظة</div>
      </div>
      <div class='stat-box'>
        <div class='v'>{len(st.session_state.docs_text)}</div>
        <div class='l'>مستندات محمّلة</div>
      </div>
      <div class='stat-box'>
        <div class='v'>{len(st.session_state.current_msgs)}</div>
        <div class='l'>رسائل الجلسة الحالية</div>
      </div>
      <div class='stat-box'>
        <div class='v'>{"✅" if st.session_state.api_key else "❌"}</div>
        <div class='l'>حالة الاتصال</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    st.markdown("---")
    ...
    st.session_state.api_key = st.text_input(
        ...
        type="password",
        placeholder="AIza... أو sk-..."
    )

    key_ok = bool(st.session_state.api_key.strip())
    col = "#4ade80" if key_ok else "#ff5a5a"
    txt = "✅ متصل — المفتاح مُدخل" if key_ok else "⚠️ غير متصل — أدخل مفتاح API"
    st.markdown(
        f"<div class='alert' style='border-right-color:{col};color:{col} !important;'>{txt}</div>",
        unsafe_allow_html=True
    )
...
if active_tool == "calculator":
    ...
    if st.button("احسب المستحقات ⚡"):
        if basic > 0 and years > 0:
            ...
            <div class='stat-grid'>
              <div class='stat-box'>
                <div class='v'>{res['eosb']:,.0f}</div>
                <div class='l'>مكافأة نهاية الخدمة</div>
              </div>
              <div class='stat-box'>
                <div class='v'>{res['arbitrary']:,.0f}</div>
                <div class='l'>تعويض تعسفي</div>
              </div>
              <div class='stat-box'>
                <div class='v'>{res['delay']:,.0f}</div>
                <div class='l'>تعويض التأخير</div>
              </div>
              <div class='stat-box' style='border-color:#c9a84c;'>
                <div class='v' style='font-size:1.5rem;'>{res['grand']:,.0f}</div>
    ...
    if st.button("🔍 فحص المراسلات الآن"):
        ...
        if combined.strip():
            ...
            if findings:
                st.markdown(f"### ⚡ اكتُشفت {len(findings)} نقطة قانونية")
                for fi in findings:
                    lvl_icon = {"danger": "🚨", "warn": "⚠️", "info": "📌"}.get(fi["level"], "📌")
                    st.markdown(f"""
                    <div class='alert {fi["level"]}'>
                      {lvl_icon} <strong>{fi["msg"]}</strong><br>
                      <small style='opacity:0.72;'>النص: «{fi["snippet"]}»</small>
                    </div>
                    """, unsafe_allow_html=True)
            else:
                st.markdown(
                    "<div class='alert ok'>✅ لم تُكتشف زلات صريحة بالأنماط الآلية.</div>",
                    unsafe_allow_html=True
                )
        else:
    ...
    if st.button("بحث 🔍") and q.strip():
        st.session_state.pending_input = (
            f"ابحث في نظام العمل السعودي عن: {q}\n"
    ...
    case_text = st.text_area(
        ...
        height=160,
        placeholder="مثال: نُقلت تعسفياً بعد تقديم شكوى رسمية، وتلقيت تقييم أداء سلبياً من مشرف لم يعمل معي فعلياً..."
    )

    if not st.session_state.api_key:
        st.markdown(
            "<div class='alert warn'>⚠️ أدخل مفتاح API أولاً من زر «الاتصال بالخادم».</div>",
            unsafe_allow_html=True
        )
    ...
    if st.button("🧭 ابدأ التحليل الشامل") and case_text.strip():
        if not st.session_state.api_key:
            ...
            def _update(i, total, name):
                progress_bar.progress(i / total, text=f"محور {i}/{total}: {name}")
                status_txt.caption(f"يحلل: {name}")

            with st.spinner("جارٍ تحليل القضية..."):
                result = analysis_engine.run_full_analysis(
                    case_text, ai_call, progress_callback=_update
                )
...
if not st.session_state.current_sid:
    ...
    <div style='text-align:center;padding:50px 16px;'>
      <div style='font-size:3.4rem;margin-bottom:12px;'>
        {'⚖️' if st.session_state.persona == 'lawyer' else '🧑'}
      </div>
      <h2 style='color:{persona_color};font-size:1.5rem;margin-bottom:8px;'>
        {persona_label}
      </h2>
      <p style='color:#8a8ca0;font-size:0.92rem;max-width:440px;margin:0 auto 20px;'>
        {'يصيغ الدعاوى، يكتب المذكرات، ويبني خط الدفاع القانوني.'
          if st.session_state.persona == 'lawyer'
          else 'يحسب المستحقات، يحلل المخاطر، ويوجّه نحو القرار الأمثل.'}
      </p>
      <p style='color:#55576a;font-size:0.82rem;'>
        ابدأ بفتح جلسة جديدة من الشريط الجانبي
      </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    for msg in st.session_state.current_msgs:
        ...
        if msg["role"] == "user":
            chat_html += (
                ...
                f"<div class='bubble-meta'>{ts}</div></div>"
            )
        else:
            chat_html += (
